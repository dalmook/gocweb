...
def _run_executable_block(block: ReportBlock) -> dict:
    config = _safe_json(block.config_json)
    if block.block_type == "python":
        timeout = int(config.get("timeout_sec") or 300)
        return run_python_block(block.source_code_text or "", config, timeout_sec=timeout)
    if block.block_type == "sql":
        return run_sql_block(block.source_code_text or "", config, ensure_run_dir(0))
    return {"status": "skipped", "summary": "Skipped", "content_html": "", "content_text": "", "error_text": "", "attachments": []}
...
def run_block(db: Session, block_id: int, run_type: str = "manual") -> RunHistory:
    ...
    try:
        ...
        if block.block_type == "python":
            ...
            payload = {"status": "skipped", "summary": "skipped"}
    except Exception:
        payload = {
            "status": "failed",
...
def run_page_and_create_snapshot(
    ...
    for block in exec_blocks:
        block_started = datetime.utcnow()
        try:
            run_log = run_block(db, block.id, run_type=run_type)
            ...
            payload = {
                ...
                "attachments": run_log.attachments,
            }
        except Exception:
            status = "failed"
            payload = {
        ...
        block_finished = datetime.utcnow()
        content_joined = (payload.get("content_html") or "") + "\n" + (payload.get("content_text") or "")
        bs = BlockSnapshot(
            ...
            finished_at=block_finished,
            duration_ms=int((block_finished - block_started).total_seconds() * 1000),
            content_hash=hashlib.sha256(content_joined.encode("utf-8")).hexdigest(),
            summary_hash=hashlib.sha256((payload.get("summary", "") or "").encode("utf-8")).hexdigest(),
        )
        db.add(bs)
        db.commit()
        db.refresh(bs)
...
def get_snapshot_blocks(db: Session, snapshot_id: int):
    return db.scalars(
        select(BlockSnapshot).where(BlockSnapshot.page_snapshot_id == snapshot_id).order_by(BlockSnapshot.id.asc())
    ).all()
...
def summarize_snapshot_status(snapshot: PageSnapshot) -> dict:
    ...
    return {
        "success": len([s for s in statuses if s == "success"]),
...
def compare_snapshot_with_previous(db: Session, snapshot_id: int) -> list[dict]:
    ...
    previous = db.scalars(
        select(PageSnapshot)
        .where(PageSnapshot.page_id == current.page_id, PageSnapshot.started_at < current.started_at)
        .order_by(PageSnapshot.started_at.desc())
        .limit(1)
    ).first()
...
def count_entities(db: Session) -> dict:
    return {
        ...
        or 0,
        "scheduled_pages": db.scalar(
            select(func.count()).select_from(ReportPage).where(ReportPage.schedule_enabled.is_(True), ReportPage.is_active.is_(True))
        )
        or 0,
    }

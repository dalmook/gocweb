...
def view_home(request: Request, q: str = Query(""), db: Session = Depends(get_db)):
    categories, counts = get_active_categories_for_view(db)
    recent_snapshots = db.scalars(select(PageSnapshot).where(PageSnapshot.is_published.is_(True)).order_by(PageSnapshot.started_at.desc()).limit(10)).all()
    recent_pages = db.scalars(select(ReportPage).where(ReportPage.is_active.is_(True), ReportPage.is_archived.is_(False)).order_by(ReportPage.updated_at.desc()).limit(8)).all()
    ...
    return templates.TemplateResponse(
        "view/home.html",
        {
            ...
            "freshness": freshness,
            **build_view_common_context(db, q),
        },
    )


@router.get("/{category_slug}", response_class=HTMLResponse)
def view_category(category_slug: str, request: Request, q: str = Query(""), db: Session = Depends(get_db)):
    ...
    for p in pages:
        ...
        page_meta[p.id] = {"status": status, "latest": latest, "block_count": len([b for b in p.blocks if b.is_active and not b.is_archived])}

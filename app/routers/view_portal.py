...
def view_home(request: Request, db: Session = Depends(get_db)):
    ...
    recent_pages = db.scalars(
        select(ReportPage).where(ReportPage.is_active.is_(True)).order_by(ReportPage.updated_at.desc()).limit(8)
    ).all()
...
def view_category(category_slug: str, request: Request, db: Session = Depends(get_db)):
    ...
    for p in pages:
        ...
        page_meta[p.id] = {
            "status": status,
            "latest": latest,
            "block_count": len([b for b in p.blocks if b.is_active]),
        }

    return templates.TemplateResponse(
        "view/category.html",
        {"request": request, "category": category, "pages": pages, "page_meta": page_meta},
    )
...
def view_page(
    ...
    context = build_view_page_context(db, page.id, selected_run_id=run_id, history_limit=14)
